...
const columns: any = [
  ...
  },
  {
    ...
    align: 'center',
    label: 'Активность',
    field: (row: any) => { return row.active ? 'Да' : 'Нет' },
    sortable: false
  },
  {
    name: 'Дата создания',
...
const rowClick = (_evt: any, row: any) => {
  router.push(`/admin/user/${row.userId}`)
}
...
const loadData = async () => {
  ...
  await apiFetch(`/api/admin/users/${pagination.value.page}`)
    .then((result: any) => {
...
const doDelete = async () => {
  ...
  const userIds = selected.value.map((row: any) => row.userId)

  await apiFetch('/api/admin/users/destroy', {
    method: 'post',
    body: {
      userIds
    }

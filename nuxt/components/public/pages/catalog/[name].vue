<template>
  ...
  <div class="detail-section">
    ...
    >
      <div
        v-if="component.type === 'catalogMain'"
      ...
      >
        <div class="detail-section__container__left">
          ...
          >
            {{ (component.props as CatalogMainComponent)?.title }}
          </h1>

          <PublicWidgetsCarouselCatalog
            :images="(component.props as CatalogMainComponent)?.images"
          />

          <PublicWidgetsCatalogSideList
            :rating="(component.props as CatalogMainComponent)?.rating"
            class="detail-section__container__left__onlylaptop"
          />
          <PublicWidgetsCatalogSideBlock
            :data="(component.props as CatalogMainComponent)?.plusesAndMinuses"
            class="detail-section__container__left__onlylaptop"
          />
        </div>
        ...
        <div class="detail-section__container__right">
          ...
          <div class="detail-section__container__right__details">
            <PublicWidgetsCatalogDetailsList
              :year="car.fields"
              :car-data="component.props as CatalogMainComponent"
            />
          </div>
          ...
          <PublicSharedUiCatalogList
            :data="(component.props as CatalogMainComponent)?.recommendation"
          />
  ...
  <PublicWidgetsCommonFormModal v-model:show-modal="showModal" />
</template>

<script setup lang="ts">
import type { TCar, CatalogMainComponent } from '~/components/public/entities/CatalogCars/types'
...
const { getCar } = useCatalogStore()
const route = useRoute()
const router = useRouter()
const car = ref<TCar>({} as TCar)
const { data, error } = await useAsyncData(
  'car',
  async () => await getCar({ url: String(`catalog/${route.params.name}`) })
)
